$("#botonenviar").click(function() {
    if ($("#FormAtributo").valid()) {
        existeAtributo6($("#inputAtributo").val());
    }
});


function existeAtributo6(Atributo) {
    var urlApi = localStorage.getItem("urlApi");
    var settings = {
        "url": urlApi+'CantidadAtributo6V1/',
        "method": "POST",
        "headers": {
...
function ejecutarAgregarAtributo6() {
    if ($("#FormAtributo").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": localStorage.getItem("urlApi")+'addNewAtributo6V1',
            "method": "POST",
            "headers": {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": "Bearer " + localStorage.getItem('Token')
        ...
        $.ajax(settings).done(function(response) {
            ...
            Bitacora(localStorage.getItem("IdUsuario"),localStorage.getItem("IP"),"Nuevo Atributo6: "+$("#inputAtributo").val(),0,"C");
            const Toast = Swal.mixin({
                toast: true,
                position: 'top-end',
                showConfirmButton: false,
...
function cargarTablaDiccionarioExistente(Item,idCategoria){
    ...
    $("#idBotonAgregarAtributo6").prop('disabled', false);
    $('#TableDiccionarioExistente').dataTable({
        ...
        //"deferRender":  true,
        "ajax": {
            "url": localStorage.getItem("urlApi")+'getAtributo6XNombreAtributo6V1/',
            "type": "POST",
            "headers": {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": "Bearer " + localStorage.getItem('Token')
        ...
        "createdRow": function( row, data, dataIndex){
            if (data['Id_Categoria'] == idCategoria){
                ...
                $("#idBotonAgregarAtributo6").prop('disabled', true);
...
function deleteAction(data) {
    var msg;
    var settings = {
        "url": localStorage.getItem("urlApi")+'getCantidadProducto4IdAtributo6V1/'+data,
        "method": "get",
        "headers": {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": "Bearer " + localStorage.getItem('Token')
...
function deleteAtributo6(data) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var settings = {
                "url": localStorage.getItem("urlApi")+'deleteAtributo6V1/' + data,
                "method": "get",
                "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
            ...
            $.ajax(settings).done(function(response) {
                Bitacora(localStorage.getItem("IdUsuario"),localStorage.getItem("IP"),"Borrar Atributo 6 (IdAtributo6)",data,"D");
...
function ActualizarRegistro() {
    if ($("#FormAtributoEdit").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": localStorage.getItem("urlApi")+'updateAtributo6V1',
            "method": "post",
            "headers": {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": "Bearer " + localStorage.getItem('Token')
        ...
        $.ajax(settings).done(function(response) {
            Bitacora(localStorage.getItem("IdUsuario"),localStorage.getItem("IP"),"Actualizar Atributo6 (IdAtributo6)",$("#inputIdEditAtributo").val(),"U");
            let xtable = $('#TableAtributo').DataTable();
            xtable.ajax.reload(null, false);
            const Toast = Swal.mixin({
                toast: true,
...
function EditAction(data) {
    ...
    var settings = {
        "url": localStorage.getItem("urlApi")+'getAtributo6IdV1/' + data,
        "method": "get",
        "headers": {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": "Bearer " + localStorage.getItem('Token')
...
function cargarTabla(Id){
    Bitacora(localStorage.getItem("IdUsuario"),localStorage.getItem("IP"),"Consulta Tabla Atributo6 (IdCategoria)",Id,"R");
    $('#TableAtributo').dataTable({
        ...
        "buttons": [
            {
                extend: 'excelHtml5',
                title: 'Diccionario Atributo 6 - '+$('select[name="selectCategoriaTabla"] option:selected').text()
            }
        ],
        "bPaginate":    false,
        "scrollY":      400,
        ...
        "deferRender":  true,
        "ajax": {
            "url": localStorage.getItem("urlApi")+'getAtributo64CategoriaV1/'+Id,
            "type": "GET",
            "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
