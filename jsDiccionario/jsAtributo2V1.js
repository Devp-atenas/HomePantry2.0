$("#botonenviar").click(function() {
    if ($("#FormAtributo").valid()) {
        existeAtributo2($("#inputAtributo").val());
    }
});


function existeAtributo2(Atributo) {
    var urlApi = localStorage.getItem("urlApi");
    var settings = {
        "url": urlApi+'CantidadAtributo2V1/',
        "method": "POST",
        "headers": {
...
function ejecutarAgregarAtributo2() {
    if ($("#FormAtributo").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": localStorage.getItem("urlApi")+'addNewAtributo2V1',
            "method": "POST",
            "headers": {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": "Bearer " + localStorage.getItem('Token')
        ...
        $.ajax(settings).done(function(response) {
            ...
            Bitacora(localStorage.getItem("IdUsuario"),localStorage.getItem("IP"),"Nuevo Atributo2: "+$("#inputAtributo").val(),0,"C");
            const Toast = Swal.mixin({
                toast: true,
                position: 'top-end',
                showConfirmButton: false,
...
function cargarTablaDiccionarioExistente(Item,idCategoria){
    ...
    $("#idBotonAgregarAtributo2").prop('disabled', false);
    $('#TableDiccionarioExistente').dataTable({
        ...
        //"deferRender":  true,
        "ajax": {
            "url": localStorage.getItem("urlApi")+'getAtributo2XNombreAtributo2V1/',
            "type": "POST",
            "headers": {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": "Bearer " + localStorage.getItem('Token')
        ...
        "createdRow": function( row, data, dataIndex){
            if (data['Id_Categoria'] == idCategoria){
                ...
                $("#idBotonAgregarAtributo2").prop('disabled', true);
...
function deleteAction(data) {
    ...
    var msg;
    var settings = {
        "url": localStorage.getItem("urlApi")+'getCantidadProducto4IdAtributo2V1/'+data,
        "method": "get",
        "headers": {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": "Bearer " + localStorage.getItem('Token')
...
function deleteAtributo2(data) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var settings = {
                "url": localStorage.getItem("urlApi")+'deleteAtributo2V1/' + data,
                "method": "get",
                "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
            ...
            $.ajax(settings).done(function(response) {
                Bitacora(localStorage.getItem("IdUsuario"),localStorage.getItem("IP"),"Borrar Atributo 2 (IdAtributo2)",data,"D");
...
function ActualizarRegistro() {
    if ($("#FormAtributoEdit").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": localStorage.getItem("urlApi")+'updateAtributo2V1',
            "method": "post",
            "headers": {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": "Bearer " + localStorage.getItem('Token')
        ...
        $.ajax(settings).done(function(response) {
            Bitacora(localStorage.getItem("IdUsuario"),localStorage.getItem("IP"),"Actualizar Atributo2 (IdAtributo2)",$("#inputIdEditAtributo").val(),"U");
            let xtable = $('#TableAtributo').DataTable();
            xtable.ajax.reload(null, false);
            const Toast = Swal.mixin({
                toast: true,
...
function EditAction(data) {
    ...
    var settings = {
        "url": localStorage.getItem("urlApi")+'getAtributo2IdV1/' + data,
        "method": "get",
        "headers": {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": "Bearer " + localStorage.getItem('Token')
    ...
    $.ajax(settings).done(function(response) {
        ...
        Bitacora(localStorage.getItem("IdUsuario"),localStorage.getItem("IP"),"Consulta Atributo2 (IdAtributo2)",data,"R");
...
function cargarTabla(Id){
    ...
    $('#TableAtributo').dataTable({
        ...
        "buttons": [
            {
                extend: 'excelHtml5',
                title: 'Diccionario Atributo 2 - '+$('select[name="selectCategoriaTabla"] option:selected').text()
            }
        ],
        "bPaginate":    false,
        "scrollY":      400,
        ...
        "deferRender":  true,
        "ajax": {
            "url": localStorage.getItem("urlApi")+'getAtributo24CategoriaV1/'+Id,
            "type": "GET",
            "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
