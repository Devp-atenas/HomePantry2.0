...
$("#botonenviar").click(function() {
    if ($("#FormFabricante").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": localStorage.getItem("urlApi")+'addNewFabricante',
            "method": "POST",
            "headers": {
...
function deleteAction(data) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var settings = {
                "url": localStorage.getItem("urlApi")+'deleteFabricante/' + data,
                "method": "get",
                "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": 'bearer ' + localStorage.getItem('Token')
...
function ActualizarRegistro() {
    if ($("#FormFabricanteEdit").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": localStorage.getItem("urlApi")+'updateFabricante',
            "method": "post",
            "headers": {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": 'bearer ' + localStorage.getItem('Token')
...
function EditAction(data) {
    ...
    var settings = {
        "url": localStorage.getItem("urlApi")+'getFabricanteId/' + data,
        "method": "get",
        "headers": {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": 'bearer ' + localStorage.getItem('Token')
...
function cargarTabla(Id){
    Bitacora(localStorage.getItem("IdUsuario"),localStorage.getItem("IP"),"Consulta Tabla Fabricantes (IdCategoria)",Id,"C");
    $('#TableFabricante').dataTable({
        ...
        "deferRender":  true,
        "ajax": {
            "url": localStorage.getItem("urlApi")+'getAllFabricante4Categoria/'+Id,
            "type": "GET",
            "headers": {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": "Bearer " + localStorage.getItem('Token')
