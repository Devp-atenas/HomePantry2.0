$("#botonenviar").click(function() {
    if ($("#FormAtributo").valid()) {
        existeAtributo7($("#inputAtributo").val());
    }
});
...
function existeAtributo7(Atributo) {
    ...
    var settings = {
        "url": urlApi+'CantidadAtributo7V1/',
        "method": "POST",
        "headers": {
...
function ejecutarAgregarAtributo7() {
    if ($("#FormAtributo").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
...
function cargarTablaDiccionarioExistente(Item,idCategoria){
    ...
    $('#TableDiccionarioExistente').dataTable({
        ...
        //"deferRender":  true,
        "ajax": {
            "url": localStorage.getItem("urlApi")+'getAtributo7XNombreAtributo7V1/',
            "type": "POST",
            "headers": {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": "Bearer " + localStorage.getItem('Token')
        ...
        "createdRow": function( row, data, dataIndex){
            if (data['Id_Categoria'] == idCategoria){
                ...
                $("#idBotonAgregarAtributo7").prop('disabled', true);
...
function deleteAction(data) {
    var msg;
    var settings = {
        "url": localStorage.getItem("urlApi")+'getCantidadProducto4IdAtributo7V1/'+data,
        "method": "get",
        "headers": {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": "Bearer " + localStorage.getItem('Token')
...
function cargarTabla(Id){
    ...
    $('#TableAtributo').dataTable({
        ...
        "buttons": [
            {
                extend: 'excelHtml5',
                title: 'Diccionario Atributo 7 - '+$('select[name="selectCategoriaTabla"] option:selected').text()
            }
        ],
        "bPaginate":    false,
        "scrollY":      400,
