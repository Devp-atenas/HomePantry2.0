$("#botonenviar").click(function() {
    if ($("#FormTamRango").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": localStorage.getItem("urlApi")+'addNewTamRango',
            "method": "POST",
            "headers": {
...
function deleteAction(data) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var settings = {
                "url": localStorage.getItem("urlApi")+'deleteTamRango/' + data,
                "method": "get",
                "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
...
function ActualizarRegistro() {
    if ($("#FormTamRangoEdit").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": localStorage.getItem("urlApi")+'updateTamRango',
            "method": "post",
            "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
...
function EditAction(data) {
    ...
    var settings = {
        "url": localStorage.getItem("urlApi")+'getTamRangoId/' + data,
        "method": "get",
        "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
...
function cargarTabla(Id){
    ...
    $('#TableTamRango').dataTable({
        ...
        "deferRender":  true,
        "ajax": {
            "url": localStorage.getItem("urlApi")+'getTamRango4Categoria/'+Id,
            "type": "GET",
            "headers": {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": "Bearer " + localStorage.getItem('Token')
