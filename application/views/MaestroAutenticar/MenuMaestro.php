...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1><i class="fas fa-list-ol"></i>&nbsp;Maestro Menu:</h1>
            </div>
            
        </div>
    </div><!-- /.container-fluid -->
</section>
<!-- Main content Agregar Atributo 21111-->
            ...
            <div class="col-md-12">
                <div class="card card-primary">
                    ...
                    <div class="card-body" style="display: none;">
                        <form id="FormMaestroMenu">
                        <div class="form-group row mb-0 mt-0">
                                <div class="col-md-4">
                                    <div class="inputText font-weight-bold">Modulo:</div>
                                    <select id="selectModulo" name="selectModulo" class="form-control">
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    <div class="inputText font-weight-bold">Opcion:</div>
                                    <select id="selectOpciones" name="selectOpciones" class="form-control">
                                    </select>
                                </div>
                                <div class="col-md-4">
                            ...
                            <div class="form-group row mb-0 mt-0">
                               <div class="col-md-4">
                                    <div class="inputText font-weight-bold">Enlace:</div>
                                    <input type="text" name="inputAtributo" id="inputAtributo" class="form-control">
                                </div>
                                <div class="col-md-4">
                                    <div class="inputText font-weight-bold">Icono:</div>
                                    <input type="text" name="inputAtributo" id="inputAtributo" class="form-control">
                                </div>
                            </div>
            ...
            <div class="card card-primary">
                ...
                <div class="card-body">
                    <div class="card">
                        ...
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="TableAtributo" class="table table-bordered table-striped table-sm">
                                <thead>
                                    <tr>
                                        <th>Categoria</th>
                                        <th>Atributo</th>
...
$(document).ready(function() {
    ...
    cargarOpcionesMenu("#selectModulo",0);
    
    $("#selectModulo").change(function() {
        var id_modulo = $("#selectModulo").val();
        cargarOpcionesMenu("#selectOpciones",id_modulo);
    });
    
    
    $('#FormMaestroMenuEdit').validate({
        rules: {
            inputCategoriaEdit: {
                required: true,
            },
    ...
    document.getElementById('FormMaestroMenu').reset();
    $('#TableAtributo').dataTable({
        ...
        "ajax": {
            "url": '<?php echo urlApi; ?>getAllAtributo2',
            "type": "GET",
            "headers": {
...
function deleteAction(data) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var settings = {
                "url": '<?php echo urlApi; ?>deleteAtributo2/' + data,
                "method": "get",
                "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
...
function ActualizarRegistro() {
    if ($("#FormMaestroMenuEdit").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": '<?php echo urlApi; ?>updateAtributo2',
            "method": "post",
            "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
        ...
        $.ajax(settings).done(function(response) {
            ...
            var form = document.querySelector('#FormMaestroMenuEdit');
            form.reset();
...
function EditAction(data) {
    document.getElementById('FormMaestroMenuEdit').reset();
    var settings = {
        "url": '<?php echo urlApi; ?>getAtributo2Id/' + data,
        "method": "get",
        "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
...
function cargarOpcionesMenu(etiqueta,id_padre) {
    var settings = {
        "url": '<?php echo urlApi; ?>getOpcionesMenu/'+id_padre,
        "method": "get",
        "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
    ...
    $.ajax(settings).done(function(response) {
        ...
        for (var i = 0; i < response.data.length; i++) {
            select.append("<option value=" + response.data[i].id_menu + ">" + response.data[i]
                .id_menu + "-" + response.data[i].nombre + "</option>");
        }
    }).fail(function(jqXHR, textStatus) {
        if (jqXHR.status == 400) {
            const Toast = Swal.mixin({
...
$("#botonenviar").click(function() {
    if ($("#FormMaestroMenu").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": '<?php echo urlApi; ?>addNewAtributo2',
            "method": "POST",
            "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
