...
function EditAction(data) {
    var settings = {
        "url": '<?php echo urlApi; ?>getTelevisoresId/' + data,
        "method": "get",
        "headers": {
...
function deleteAction(data) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var settings = {
                "url": '<?php echo urlApi; ?>deleteTelevisores/' + data,
                "method": "get",
                "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
...
function ActualizarRegistro() {
    if ($("#FormNSEEdit").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": '<?php echo urlApi; ?>updateTelevisores',
            "method": "post",
            "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
...
$(document).ready(function() {
    ...
    $("#botonenviar").click(function() {
        if ($("#FormNSE").valid()) {
            var settings = {
                "async": true,
                "crossDomain": true,
                "url": '<?php echo urlApi; ?>addTelevisores',
                "method": "POST",
                "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
    ...
    $('#TableCategoria').dataTable({
        ...
        "ajax": {
            "url": '<?php echo urlApi;?>getAllTelevisores2',
            "type": "GET",
            "headers": {
                /*'Authorization': 'bearer ' + localStorage.getItem('Token')*/
            },
