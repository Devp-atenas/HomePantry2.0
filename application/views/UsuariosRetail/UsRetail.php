...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1><i class="fas fa-users"></i>&nbsp; Mantenimiento de Usuarios Retail:</h1>
            </div>
            <div class="col-sm-6">
        ...
        <div class="col-md-12">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Usuarios Retail</h3>
                ...
                <div class="card-body" style="display: none;">
                    <form id="FormUsuariosRetail">
                        ...
                        <div class="form-group">
                            <label for="inputRetail">Cliente:</label>
...
<section class="content">
    <div class="row">
        <div class="col-md-12">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Listados de Usuarios Retail</h3>
                    <div class="card-tools">
                        <button type="button" class="btn btn-tool" data-card-widget="collapse" title="Collapse">
                ...
                <div class="card-body">
                    <div class="card">
                        <div class="card-body">
                            <table id="TableUsuarios" class="table table-bordered table-striped table-sm">
                                <thead>
                                    <tr>
                                        <th>Nombres</th>
                                        <th>Apellidos</th>
            ...
            <div class="modal-body">
                <div class="col-md-12">
                    <div class="card card-primary">
                        ...
                        <div class="card-body">
                            <form id="FormUsuariosRetailEdit">
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    <label for="inputEstatusEdit">Estatus:</label>
                                    <select id="inputEstatusEdit" name="inputEstatusEdit" class="form-control">
                                    </select>
                                </div>
                                <div class="form-group">
                                    <div class="row">
                ...
                <div class="col-md-12">
                    <div class="card card-primary">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                <label for="inputRetailVer">Cliente:</label>
...
$(document).ready(function() {
    document.getElementById('FormUsuariosRetail').reset();
    document.getElementById('FormUsuariosRetailEdit').reset();
    //Validacion del Formularios de Nuevo Usuarios
    $('#FormUsuariosRetail').validate({
        rules: {
            inputNombres: {
                required: true,
            },
    ...
    $('#FormUsuariosRetailEdit').validate({
        rules: {
            inputNombresEdit: {
                required: true,
            },
    ...
    $("#botonenviar").click(function() {
        if ($("#FormUsuariosRetail").valid()) {
            var settings = {
                "async": true,
                "crossDomain": true,
                "url": '<?php echo urlApi; ?>addNewUsRetail',
                "method": "POST",
                "headers": {
            ...
            $.ajax(settings).done(function(response) {
                ...
                var form = document.querySelector('#FormUsuariosRetail');
                form.reset();
    ...
    $('#TableUsuarios').dataTable({
        ...
        "ajax": {
            "url": '<?php echo urlApi; ?>getAllUsersRetail',
            "type": "GET",
            "headers": {
                'Authorization': 'bearer ' + localStorage.getItem('Token')
            },
...
function cargarNiveles() {
    var settings = {
        "url": '<?php echo urlApi; ?>getNivelRetail',
        "method": "get",
        "headers": {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": "Bearer " + localStorage.getItem('Token')
...
function cargarRetailEdit(idCliente) {
    var settings = {
    ...
    $.ajax(settings).done(function(response) {
        ...
        for (var i = 0; i < response.data.length; i++) {
            if (response.data[i].Id_Cliente == idCliente) {
                selectRetailEdit.append("<option value=" + response.data[i].Id_Cliente + " selected>" +
                    response.data[i].Cliente + "</option>");
            } else {
                selectRetailEdit.append("<option value=" + response.data[i].Id_Cliente + ">" + response.data[
                    i].Cliente + "</option>");
            }
        }
    }).fail(function(jqXHR, textStatus) {
...
function cargarEstatusEdit(estatus) {
    let select = $("#inputEstatusEdit");
    select.find("option").remove();
    if (estatus=='Inactivo'){
        select.append("<option value=1>Activo</option>");
        select.append("<option value=0 selected>Inactivo</option>");
    }else{
        select.append("<option value=1 selected>Activo</option>");
...
function cargarRetail() {
    ...
    $.ajax(settings).done(function(response) {
        ...
        select.append("<option value='' selected disabled> -- Seleccione -- </option>");
        for (var i = 0; i < response.data.length; i++) {
            select.append("<option value=" + response.data[i].Id_Cliente + ">" + response.data[i].Cliente +
                "</option>");
        }
    }).fail(function(jqXHR, textStatus) {
        if (jqXHR.status == 400) {
...
function deleteAction(data) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var settings = {
                "url": '<?php echo urlApi; ?>deleteUsRetail/' + data,
                "method": "get",
                "headers": {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": "Bearer " + localStorage.getItem('Token')
...
function EditAction(data) {
    var settings = {
        "url": '<?php echo urlApi; ?>searchUsRetail/' + data,
        "method": "get",
        "headers": {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": "Bearer " + localStorage.getItem('Token')
    ...
    $.ajax(settings).done(function(response) {
        ...
        cargarRetailEdit(response.data[0].Id_Cliente);
        cargarEstatusEdit(response.data[0].estatus);
...
function ActualizarRegistro() {
    if ($("#FormUsuariosRetailEdit").valid()) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": '<?php echo urlApi; ?>updateUsRetail',
            "method": "post",
            "headers": {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": "Bearer " + localStorage.getItem('Token')
        ...
        $.ajax(settings).done(function(response) {
            ...
            var form = document.querySelector('#FormUsuariosRetailEdit');
            form.reset();
